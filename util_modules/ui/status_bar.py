    ...
    with st.sidebar:
        ...
        try:
            ...
            <div style="
                ...
                margin-bottom: 0.5rem;
            ">
                ⚕️ SCT Codes: {stats['clinical_count']:,}
            </div>
            """, unsafe_allow_html=True)
            st.markdown(f"""
            <div style="
                ...
                margin-bottom: 0.5rem;
            ">
                💊 Medications: {stats['medication_count']:,}
            </div>
            """, unsafe_allow_html=True)
            ...
            with st.sidebar.expander("🎯 What's New: v2.2.3", expanded=False):
                st.markdown("""
                    **🚀 Major Architecture & Feature Improvements - v2.2.3:**
                    - Patient Demographics LSOA Filtering: Full support for demographic filtering with future-proof year detection
                    - Export System Centralisation: Lazy loading architecture with memory leak prevention and consistent two-click workflow
                    - New Theme: Dark interface with RAG color ratings
                    - Updated Fragment Implementation: Selective rerendering for export functionality eliminating unnecessary full app reruns
                    - Enhanced Patient Demographics Support: Proper UI rendering for demographics-only XMLs and criteria phrasing
                    - Centralised Export Management: All export functionality now unified with automatic cleanup and memory optimisation
                    - Updated Interface: Comprehensive theming across all components with configurable sidebar width and logo integration
                    """)

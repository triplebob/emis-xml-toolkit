...
- **CODE_DESCRIPTION**: User-friendly code descriptions
...
### Organisation and Practice Data
- **ORGANISATION_TERM**: Practice/organisation names
...
- **EQ**: Equal to (default behaviour if no operator specified)
...
- **Parent Type**: `ALL` (organisational reporting across all patients including left and deceased)
- **Purpose**: Quality monitoring, compliance tracking, and organisational aggregation

#### Two Types of Audit Reports

**1. Simple Audit Reports (PATIENTS table)**
- **Logical Table**: `PATIENTS` 
- **Structure**: Pure organisational aggregation without additional clinical criteria
- **Example**: Basic practice population counts grouped by organisation
...
- **Purpose**: Break down results by organisational/workflow attributes
- **Common Columns**: 
...
### Author and Organisation Combined
...
- **Purpose**: Audit reports requiring active user validation across organisational boundaries

### Enhanced Column Output Types
- **Author + Organisation**: `<column>AUTHOR</column><column>NATIONAL_PRACTICE_CODE</column>` combined
...
## Enterprise and Organisational Features

### Folder Structures
- **Hierarchical Organisation**: Parent-child folder relationships
...
- **Multi-Organisation**: `<association><organisation>guid</organisation></association>` (multiple associations)
- **Version Independence**: `<VersionIndependentGUID>` for cross-version compatibility

### Organisational Association
- **Practice Codes**: Organisation GUID associations
...
- **Multi-Organisation**: Multiple `<association><organisation>guid</organisation></association>`
...
- **Usage**: Complex sorting on User Details, Concept ID, Date for organised output
...
- **Memory Optimisation**: Large file processing with streaming parsers
- **Cross-Table Joins**: Efficient relationship resolution
- **Complex Calculations**: Age and date arithmetic optimisation
...
5. **Scope**: Multi-organisation vs single practice focus

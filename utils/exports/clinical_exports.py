...
def _generate_csv_with_metadata(filtered_df: pd.DataFrame, xml_label: str) -> str:
    ...
    metadata_lines = [
        ...
        f"# Generated by: ClinXML (https://clinxml.streamlit.app/)",
...
def render_export_controls(
    ...
    with col1:
        ...
        st.caption(f"📊 Total: {total} | ✅ Matched: {matched} | ❌ Unmatched: {unmatched}")
    ...
    with col2:
        ...
        if is_ready and stored_path and Path(stored_path).exists():
            ...
            btn_label = f"📥 Download {filter_label} {base_label} ({mode_label})"
            ...
            btn_label = f"🔄 Export {filter_label} {base_label} ({mode_label})"
        ...
        st.caption(f"Will generate CSV with {filtered_count} rows × {len(export_df.columns)} columns")

...
def render_audit_reports_tab(xml_content: str, xml_filename: str):
    ...
    if not xml_content:
        st.markdown(info_box("📊 Upload and process an XML file to see Audit Reports"), unsafe_allow_html=True)
        return
    ...
    try:
        ...
        display_generic_error(f"Error analyzing Audit Reports: {str(e)}", "error")
...
def render_audit_report_details(report):
    ...
    if analysis:
        ...
        if member_searches:
            ...
            st.markdown(info_box("This Audit Report combines results from the following base searches:"), unsafe_allow_html=True)

...
def _batch_lookup_snomed_for_ui(emis_guids: List[str]) -> Dict[str, str]:
    ...
    try:
        ...
        cached_unified_data = st.session_state.get(SessionStateKeys.UNIFIED_CLINICAL_DATA_CACHE)
...
def classify_report_type_cached(report_id: str, report_structure_hash: str) -> str:
    ...
    analysis = st.session_state.get(SessionStateKeys.XML_STRUCTURE_ANALYSIS) or st.session_state.get(SessionStateKeys.SEARCH_ANALYSIS)
...
def ensure_analysis_cached(xml_content=None):
    ...
    xml_structure_analysis = st.session_state.get(SessionStateKeys.XML_STRUCTURE_ANALYSIS)

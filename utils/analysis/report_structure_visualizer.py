...
def render_folder_structure(folder_tree, folders, reports):
    ...
    with st.expander("📥 Export Folder Structure", expanded=False):
        ...
        with col1:
            ...
            def folder_txt_export_fragment():
                ...
                xml_filename = st.session_state.get(SessionStateKeys.XML_FILENAME, 'unknown')
                ...
                if f'{SessionStateKeys.TREE_TEXT_PREFIX}{cache_key}' not in st.session_state:
                    st.session_state[f'{SessionStateKeys.TREE_TEXT_PREFIX}{cache_key}'] = generate_folder_tree_ascii(folder_tree, folder_map, report_map)
                
                # Build TXT content with summary
                txt_content = f"Folder Structure - Generated {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                txt_content += "=" * 80 + "\n\n"
                txt_content += st.session_state[f'{SessionStateKeys.TREE_TEXT_PREFIX}{cache_key}']
                txt_content += "\n\n" + "=" * 80 + "\n"
        ...
        with col2:
            ...
            def folder_json_export_fragment():
                ...
                if f'{SessionStateKeys.TREE_JSON_PREFIX}{cache_key}' not in st.session_state:
                    ...
                    st.session_state[f'{SessionStateKeys.TREE_JSON_PREFIX}{cache_key}'] = (filename, json_content)
                
                # Get cached JSON data
                filename, json_content = st.session_state[f'{SessionStateKeys.TREE_JSON_PREFIX}{cache_key}']
    ...
    if f'{SessionStateKeys.TREE_TEXT_PREFIX}{cache_key}' not in st.session_state:
        st.session_state[f'{SessionStateKeys.TREE_TEXT_PREFIX}{cache_key}'] = generate_folder_tree_ascii(folder_tree, folder_map, report_map)
    st.code(st.session_state[f'{SessionStateKeys.TREE_TEXT_PREFIX}{cache_key}'], language="")
...
def render_dependency_tree(dependency_tree, reports):
    ...
    if f'{SessionStateKeys.DEP_TREE_TEXT_PREFIX}{cache_key}' not in st.session_state:
        st.session_state[f'{SessionStateKeys.DEP_TREE_TEXT_PREFIX}{cache_key}'] = generate_dependency_tree_ascii(dependency_tree, report_map, show_circular)
    ...
    with st.expander("📥 Export Dependencies", expanded=False):
        ...
        with col1:
            ...
            def dependency_txt_export_fragment():
                # TXT export using cached tree text
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                xml_filename = st.session_state.get(SessionStateKeys.XML_FILENAME, 'dependency_tree')
                clean_xml_name = xml_filename.replace('.xml', '').replace(' ', '_')
                
                # Generate summary statistics to match JSON export
                from utils.export_handlers.json_export_generator import JSONExportGenerator
                from utils.ui.tabs.tab_helpers import ensure_analysis_cached
                ...
                dep_txt_content += "=" * 80 + "\n\n"
                dep_txt_content += st.session_state[f'{SessionStateKeys.DEP_TREE_TEXT_PREFIX}{cache_key}']
                dep_txt_content += "\n\n" + "=" * 80 + "\n"
    ...
    with st.expander("🌳 Dependency Tree", expanded=True):
        st.code(st.session_state[f'{SessionStateKeys.DEP_TREE_TEXT_PREFIX}{cache_key}'], language="")

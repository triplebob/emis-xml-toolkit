...
class SearchAnalyzer:
    ...
    def _parse_criteria_group(self, group_elem: ET.Element, namespaces: Dict) -> Optional[CriteriaGroup]:
        ...
        try:
            ...
            # Use mutually exclusive patterns to avoid duplicating the same criterion
            
            # First, try Pattern 2: Criterion elements wrapped in criteria elements (patient demographics pattern)
            criteria_elements = self.ns.findall_with_path(definition_elem, 'criteria')
            has_wrapped_criteria = False
            
            for criteria_elem in criteria_elements:
                criterion_elems = self.ns.findall_with_path(criteria_elem, 'criterion')
                if criterion_elems:
                    has_wrapped_criteria = True
                    for criterion_elem in criterion_elems:
                        criterion = self.criterion_parser.parse_criterion(criterion_elem)
                        if criterion:
                            criteria.append(criterion)
            
            # Pattern 1: Direct criterion elements under definition (only if no wrapped criteria found)
            if not has_wrapped_criteria:
